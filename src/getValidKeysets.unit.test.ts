...
it('getValidKeysets return all keysets (Proof of play)', async () => {
  const { isAccurate, keysets } = await getKeysets(client, {
...
describe('setValidKeysetFunctionSelector/invalidateKeysetFunctionSelector', () => {
  it('getValidKeysets return all keysets with isAccurate flag set to true', async () => {
    const mockTransport = () =>
      createTransport({
        key: 'mock',
        name: 'Mock Transport',
    ...
    const { keysets, isAccurate } = await getKeysets(mockClient, {
      ...
      createTransport({
        ...
        request: vi.fn(({ method, params }) => {
          return mockData({
            logs: [
              mockSetValidKeysetEvent(keysetHashForZeroPK, keysetForZeroPK),
              mockSetValidKeysetEvent(keysetHash), // Missing keyset
              mockInvalidateKeysetHashEvent(keysetHash),
            ],
            method,
            params,
