...
describe('successfully get batch posters', () => {
    it('when disabling the same batch posters multiple time', async () => {
        ...
        const { isAccurate: isAccurateInitially, batchPosters: initialBatchPosters } = await getBatchPosters(
            client,
            {
                rollup: l3Rollup,
                sequencerInbox: l3SequencerInbox
        ...
        const { isAccurate: isStillAccurate, batchPosters: newBatchPosters } = await getBatchPosters(client, {
            rollup: l3Rollup,
            sequencerInbox: l3SequencerInbox
        });
        ...
        const { batchPosters, isAccurate } = await getBatchPosters(client, { rollup: l3Rollup, sequencerInbox: l3SequencerInbox });
    ...
    it('when adding an existing batch poster', async () => {
        ...
        const { isAccurate, batchPosters } = await getBatchPosters(client, { rollup: l3Rollup, sequencerInbox: l3SequencerInbox });
        expect(batchPosters).toEqual(initialBatchPosters);
        expect(isAccurate).toBeTruthy();
    });
