...
describe('max time variation management', () => {
  ...
  it('buildSetMaxTimeVariation successfully set max time varation', async () => {
    ...
    await setMaxTimeVariation(originalValues);
    const finalResult = await getMaxTimeVariation(client, {
      sequencerInbox: l3SequencerInbox,
    });
    expect(finalResult).toEqual(originalValues);
  });
});
...
describe('keyset management', () => {
  async function deployAnytrustRollupWithKeyset(keyset: Hex) {
    ...
    return {
      keysetHash: logs[0].args.keysetHash,
      sequencerInbox,
      upgradeExecutor,
    };
  }
  ...
  it('isValidKeysetHash successfully fetches whether a hash is a valid keyset hash', async () => {
    ...
    const { sequencerInbox, keysetHash } = await deployAnytrustRollupWithKeyset(
  ...
  it('successfully invalidate a keyset hash', async () => {
    const { sequencerInbox, keysetHash, upgradeExecutor } = await deployAnytrustRollupWithKeyset(
